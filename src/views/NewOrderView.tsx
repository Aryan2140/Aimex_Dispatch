...
import {
  Camera, Trash2, Check, Search, Plus, Minus, Truck, ImageOff, AlertCircle, Loader2,
} from 'lucide-react';
      ...
      <section className="card p-4">
        ...
        {labelPreview ? (
          ...
          <button onClick={captureFromCamera} className="flex w-full flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-slate-500 hover:border-blue-400 hover:text-blue-600">
            ...
            <span className="mt-0.5 text-xs text-slate-400">Required to identify the carrier automatically</span>
          </button>
        )}
        <p className="mt-2 flex items-center gap-1.5 text-xs text-slate-400">
          <ImageOff className="h-3.5 w-3.5" /> Photos upload to the server only — not saved on your device.
        </p>
      </section>
